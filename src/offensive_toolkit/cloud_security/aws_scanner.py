...
class AWSScanner:
    ...
    def __init__(self, profile_name: str | None = None, region: str = "us-east-1"):
        """
            ...
            for user in users:
                ...
                try:
                    ...
                    for key in access_keys:
                        ...
                        if key_age > 90:
                            user_issues.append(
                                f"Access key {key['AccessKeyId']} is {key_age} days old"
                            )
                            self._add_finding(
                                "iam",
                                {
                ...
                try:
                    ...
                    if attached_policies:
                        ...
                        user_issues.append(
                            f"Has directly attached policies: {', '.join(policy_names)}"
                        )
                        self._add_finding(
                            "iam",
                            {
...
MITRE ATT&CK Mapping:
  ...
  T1530: Data from Cloud Storage Object
    ...
    parser.add_argument(
        "--scan",
        nargs="+",
        choices=["iam", "s3", "ec2", "security_groups", "cloudtrail"],
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("output/aws_scan.json"),
